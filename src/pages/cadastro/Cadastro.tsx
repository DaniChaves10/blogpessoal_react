...
function Cadastro() {
  ...
  async function cadastrarNovoUsuario(e: SyntheticEvent<HTMLFormElement>){
    ...
    // Impede o envio automatico do formulário
    e.preventDefault();
    ...
    try{
      ...
      if (axios.isAxiosError(error) && error.response){
        alert(`Error ao cadastrar o usuário: ${error.response.status}`);
      } else{
        alert("Erro ao cadastrar o usuario! Verifique a conexão com a API!");
      }
    } finally{
      setIsLoading(false);
    }
  ...
  // Função par retornar para  a página de login
  function retornar(){
    navigate('/');
  }
